# -*- coding: utf-8 -*-
# ============================================================
# 愛媛県 全域：犯罪/異常行動 警戒予測（Streamlit 完成版 / 地図フェイルセーフ）
# - サイドバーUI（前提どおり維持）
# - 2019年オープンデータを自動検出・統合し概位置レイヤ表示を維持
# - CSV住所をNominatimでジオコーディング（/mnt/data にキャッシュ）→ 実点表示
# - mgpn 月齢API: v2(JSON)優先→v3、Accept/Retry/30分キャッシュ、ボタン押下時にのみ呼出
# - Overpass APIで周辺POI（駅/バス停/駐輪場/コンビニ/駐車場/公園/ATM）可視化
# - 将来30日で最大リスク日（21:00評価）を推定
# - 地図は st_folium の引数差異と描画失敗を吸収する**フェイルセーフ**で必ず表示
# - DuplicateWidgetID回避＆ st.experimental_rerun → st.rerun
# ============================================================
...
try:
    from folium.plugins import LocateControl  # 無い版がある
    HAS_LOCATE = True
except Exception:
    HAS_LOCATE = False
...
def main():
    ...
    st.caption("※ mgpnはv2(JSON)→v3の順に呼出し30分キャッシュ。住所ジオコーディングはNominatim(1秒/件)で行い、/mnt/dataにキャッシュします。POIはOverpass APIから取得。将来予測は月齢・曜日・季節ヒューリスティックの概算です。2019年犯罪データの概位置レイヤは維持しています。")
