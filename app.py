# -*- coding: utf-8 -*-
# ============================================================
# 愛媛県 全域：犯罪/異常行動 警戒予測（Streamlit 完成版 / 地図非表示対策版）
...
# - ★地図非表示の主因対策：streamlit-folium のバージョン差異（returned_objects → returns）への互換呼び出し
# ============================================================
...
# 一部の環境に LocateControl が無い場合があるため、動的 import
try:
...
.stButton>button { background: linear-gradient(135deg, #143957, #0d2a41) !important; border: 1px solid rgba(255,255,255,0.15) !important; color: #e6eef7 !important; font-weight: 700 !important; border-radius: 10px !important; padding: .5rem .75rem !important; }
...
# ---------------------------
# st_folium 互換呼び出しラッパ（地図非表示の主因：引数名差異を吸収）
# ---------------------------

def render_st_folium(map_obj, key: str, height: int, need_click: bool):
    # 新しめ: returned_objects / 古め: returns / さらに古い: 引数無し
    try:
...
def _add_common_map_ui(m: folium.Map):
    ...
    folium.TileLayer(tiles="https://{s}.tile.openstreetmap.fr/hot/{z}/{x}/{y}.png", attr="OSM HOT", name="OSM HOT", control=True).add_to(m)
...
def _plot_past_crimes(m: folium.Map, df: pd.DataFrame | None):
    ...
    for _, r in rows.iterrows():
        ...
        lat0, lon0 = latlon
        # 概位置（~100–300m）
        jitter_lat = float(rng.normal(0, 0.0010))
        jitter_lon = float(rng.normal(0, 0.0012))
        lat = lat0 + jitter_lat
        lon = lon0 + jitter_lon
        ...
        folium.Marker(location=[lat, lon], popup=folium.Popup(html, max_width=260), icon=folium.Icon(color=color, icon=icon)).add_to(cluster)
...
def render_map_selectable(lat: float, lon: float, snap: dict | None, past_df: pd.DataFrame | None):
    ...
    if snap:
        ...
        folium.Circle(location=[lat, lon], radius=radius, color=snap["color"], fill=True, fill_opacity=0.25, weight=2).add_to(m)
        popup_html = f"""
    ...
    folium.Marker([lat, lon], popup=folium.Popup(popup_html, max_width=320), draggable=True,
                  icon=folium.Icon(color="lightgray" if not snap else ("red" if snap["score"]>=75 else "orange" if snap["score"]>=50 else "blue"), icon="info-sign")).add_to(m)
    return m
...
def gemini_explain(snap: dict, now_dt: datetime) -> str | None:
    ...
    try:
        ...
        model = genai.GenerativeModel(model_name=GEMINI_MODEL, generation_config={"temperature": 0.2}, system_instruction=(
            ...
            "根拠を、過度な断定を避けて簡潔に日本語で説明してください。満月効果は限定的、気温・時間帯・降雨などの影響は統計的示唆あり、を前提に、"
...
def main():
    ...
    if "run_gemini" not in st.session_state: st.session_state.run_gemini = False
    ...
    out = render_st_folium(fmap, key="map_select", height=560, need_click=True)
    ...
    # 環境診断（折りたたみ）
    with st.expander("環境診断（地図が出ないとき用）"):
        st.write({"streamlit_folium_version": getattr(stf, "__version__", "?"), "folium_version": getattr(folium, "__version__", "?")})
        st.write("st_folium 引数の互換を自動適用しています（returned_objects/returns/引数無し）")
    ...
    if snap:
        ...
        with c2:
            ...
            render_st_folium(fmap2, key="map_result", height=500, need_click=False)
            st.markdown("</div>", unsafe_allow_html=True)
    ...
    st.caption("※ 月齢は mgpn API（ver2/3）をボタン押下時のみ呼出し、30分キャッシュで負荷を抑制。 気象APIキーが無い場合はダミー値で動作。2019年データは ./, ./data, /mnt/data を自動検出。犯罪箇所は市町村中心の概位置（100〜300mジッター）で表示します。")
